...
export default async function WorkerConversationPage({ params }: { params: { conversationId: string } }) {
  ...
  if (!conversation || conversation.workerId !== session.id) notFound()

  const customer = await prisma.user.findUnique({
    where: { id: conversation.job.customerId },
  ...
  return (
    <div className="flex h-[calc(100vh-10rem)] flex-col overflow-hidden rounded-2xl border border-earth-200 bg-white shadow-card">
      ...
      <div className="flex items-center gap-3 border-b border-earth-200 px-4 py-3">
        <Link href="/dashboard/worker/messages" className="text-earth-400 hover:text-earth-700">
          <ChevronLeft size={20} />
        </Link>
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-bold text-earth-900">{customer?.name ?? '…'}</p>
