  ...
  return (
    <div className="max-w-4xl space-y-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          <Link href="/dashboard/customer/jobs" className="text-sm text-earth-500 hover:text-brand-600">
            Back to My Jobs
          </Link>
          <h1 className="font-display text-xl font-bold text-earth-900 mt-2">{job.title}</h1>
        </div>
      ...
      <div className="card p-5 space-y-4">
        <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          {job.budget && <div><span className="text-earth-400 text-xs">Budget</span><p className="font-semibold text-sage-700 mt-0.5">{formatBWP(job.budget)}</p></div>}
          ...
          <div><span className="text-earth-400 text-xs">Urgency</span><p className="font-semibold text-earth-800 mt-0.5">{job.urgency.replace(/_/g, ' ')}</p></div>
